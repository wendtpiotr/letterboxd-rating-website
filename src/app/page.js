...
import {
    ...
    Clock, ArrowLeft, StarHalf, Info, Target, Trophy, Sparkles
} from 'lucide-react';
...
export default function RATR() {
    ...
    // Helper to get genre-specific and universal questions
    const allQuestions = [
        ...QUESTIONS.universal,
        ...(QUESTIONS[selectedGenre.toLowerCase()] || [])
    ];
    ...
    const activeQuestion = allQuestions[currentQuestion];
                            ...
                            <div className="grid lg:grid-cols-3 gap-12 items-start">
                                ...
                                <div className="lg:col-span-2 text-center lg:text-left py-12">
                                    ...
                                    <div className="space-y-6 mb-16">
                                        ...
                                        {/* FOCUS AREAS FOR INDIVIDUAL QUESTION */}
                                        {activeQuestion.focusAreas && (
                                            <motion.div
                                                initial={{ opacity: 0 }}
                                                animate={{ opacity: 1 }}
                                                className="flex flex-wrap justify-center lg:justify-start gap-2"
                                            >
                                                {activeQuestion.focusAreas.map((area) => (
                                                    <span key={area} className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest bg-green-500/10 text-green-500 border border-green-500/20 px-3 py-1 rounded-full">
                                                        <Sparkles size={10} /> {area}
                                                    </span>
                                                ))}
                                            </motion.div>
                                        )}
